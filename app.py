# app.py - QTI Engineering Workbench
...
def show_command_center():
    ...
    # KPI 1: Active Investigations (Open CAPAs)
    ...
    # KPI 2: Overdue CAPAs
    overdue_capas = len(capa_data[capa_data['due_date'] < datetime.now()])
    kpi_cols[1].metric("Overdue CAPAs", overdue_capas, delta=overdue_capas, delta_color="inverse")

    # KPI 3: New Anomalies (last 24h)
    ...
    # KPI 4: Overall Process Capability (Mock Cpk)
    # In a real app, this would be calculated from ongoing monitoring
    mock_cpk = round(np.random.uniform(1.2, 1.5), 2)
    kpi_cols[3].metric("Avg. Process Cpk", mock_cpk, delta=round(mock_cpk-1.33, 2))

    st.markdown("---")
    
    # --- Process Health & Active Investigations ---
    col1, col2 = st.columns((2, 1.5))

    with col1:
        st.subheader("Process Line Health Status")
        # Create a health score for each production line
        ...
        fig = px.treemap(line_health, path=['line_id'], values='anomaly_rate',
                         color='anomaly_rate',
                         color_continuous_scale='RdYlGn_r',
                         title="Process Line Anomaly Rate (Lower is Better)")
...
def show_process_monitoring():
    ...
    param_to_monitor = st.sidebar.selectbox("Select Parameter to Monitor:", 
                                            ('reagent_ph', 'fill_volume_ml', 'pressure_psi'))
    line_to_monitor = st.sidebar.selectbox("Select Process Line:", 
                                           process_data['line_id'].unique())
    ...
    # --- Tabbed Layout for different monitoring techniques ---
    ...
    with tab1:
        ...
        if not monitor_df.empty:
            # Using pyspc to create and evaluate the control chart
            spc = pyspc.Spc(monitor_df[param_to_monitor], chart_type='i', title=f'I-Chart for {param_to_monitor}')
            
            # Displaying the chart using st.pyplot
            st.pyplot(spc.get_fig())
            
            # Displaying the rules violations
    ...
    with tab2:
        ...
        if not monitor_df.empty:
            ...
            # Plot the results
            fig = px.line(monitor_df, x='timestamp', y='anomaly_score', 
                          title='Anomaly Score Over Time (Lower is More Anomalous)',
                          labels={'timestamp': 'Timestamp', 'anomaly_score': 'Anomaly Score'})
            # Highlight anomalies
...
def show_event_triage():
    ...
    with col1:
        ...
        new_complaint_text = st.text_area("Paste new complaint text here:", 
                                          "The vials from the latest shipment were poorly sealed and the pressure was all over the place during our validation run.",
                                          height=150)
    ...
    with col2:
        ...
        with st.form("rpn_form"):
            ...
            if submitted:
                ...
                if rpn > 125:
                    st.error(f"**Calculated RPN: {rpn}** (High Risk)")
                elif rpn > 60:
                    st.warning(f"**Calculated RPN: {rpn}** (Medium Risk)")
                else:
                    st.success(f"**Calculated RPN: {rpn}** (Low Risk)")
                
                st.caption("Use the RPN score to objectively prioritize which investigations to tackle first.")
# =================================================================================================
# CHUNK 5: RCA WORKBENCH
# =================================================================================================
@st.cache_resource
...
def show_rca_workbench():
    ...
    # For demonstration, we'll filter by material lot, a known issue in our data
    lot_to_investigate = st.sidebar.selectbox("Select Material Lot to Investigate:",
                                              process_data['material_lot'].unique())
    ...
    with tab1:
        ...
        # Using Graphviz to create the diagram
    ...
    with tab2:
        ...
        if ttest_res.pvalue < 0.05:
            st.error("Result is statistically significant (p < 0.05). There is strong evidence of a real difference between the two lots.")
        else:
            st.success("Result is not statistically significant (p >= 0.05).")
    ...
    with tab3:
        ...
        # Train model on the full dataset to find global importances
        rca_model = get_rca_model(process_data)
        features = ['reagent_ph', 'fill_volume_ml', 'pressure_psi']
        
        importances = pd.DataFrame({
            'feature': features,
            'importance': rca_model.feature_importances_
...
def show_capa_manager():
    ...
    if capa_to_verify:
        # For this demo, we simulate pre- and post-CAPA data
        # In a real app, this would be filtered from the main process data based on implementation date
        ...
        voe_df = pd.DataFrame({
            'Measurement': np.concatenate([pre_capa_data, post_capa_data]),
            'Phase': ['Pre-CAPA'] * 100 + ['Post-CAPA'] * 100
        })
        
        # Visualize the change
        fig = px.box(voe_df, x='Phase', y='Measurement', color='Phase',
                     title=f"Effectiveness of {capa_to_verify}: Pre- vs. Post-Implementation")
        st.plotly_chart(fig, use_container_width=True)
        ...
        if voe_ttest.pvalue < 0.05 and levene_test.pvalue < 0.05:
            st.success("✅ **Effective:** The implemented CAPA resulted in a statistically significant improvement in both the process mean and its consistency (variance).")
        else:
            st.error("❌ **Not Effective:** The change did not result in a statistically significant improvement. Re-investigation is required.")
# =================================================================================================
# CHUNK 7: PREDICTIVE ANALYTICS & MAIN FUNCTION
# =================================================================================================
@st.cache_resource
def get_prediction_model_and_explainer(_df):
    """Trains a model and creates a SHAP explainer."""
    model = get_rca_model(_df) # We can reuse the RCA model
    features = ['reagent_ph', 'fill_volume_ml', 'pressure_psi']
...
def show_predictive_analytics():
    ...
    with col1:
        ...
        # Make prediction
        prediction_proba = model.predict_proba(input_data)[0][1] # Probability of anomaly (class 1)
        
        st.metric("Predicted Probability of Anomaly", f"{prediction_proba:.1%}")
        if prediction_proba > 0.5:
            st.error("High risk of failure predicted for these settings.")
        else:
            st.success("Process settings are predicted to be stable.")
            
    with col2:
        st.write("eXplainable AI (XAI) Driver Analysis:")
        
        # Get SHAP values for the single prediction
        shap_values = explainer.shap_values(input_data)[1] # for class 1
        
        # Generate the force plot
        st.pyplot(shap.force_plot(explainer.expected_value[1], shap_values, 
                                  features=np.around(input_data.astype(float), 2),
                                  feature_names=['reagent_ph', 'fill_volume_ml', 'pressure_psi'],
                                  matplotlib=True, show=False))
        
        st.info("""
        **How to Read This Plot:**
        - **Base Value:** The average prediction probability across all data.
        - **Red Arrows:** Features pushing the prediction probability HIGHER (increasing risk).
        - **Blue Arrows:** Features pushing the prediction probability LOWER (decreasing risk).
        - **The final output value** is the model's prediction. This plot shows you exactly which parameters are contributing most to the predicted risk.
        """)
...
def main():
    ...
    module = st.sidebar.radio(
        "Select a Module:",
        ("QTI Command Center", "Process Monitoring", "Event Triage", 
         "RCA Workbench", "CAPA Manager", "Predictive Analytics")
    )
    ...
    # Page routing
    if module == "QTI Command Center":
        show_command_center()
    elif module == "Process Monitoring":
        show_process_monitoring()
    elif module == "Event Triage":
        show_event_triage()
    elif module == "RCA Workbench":
        show_rca_workbench()
    elif module == "CAPA Manager":
        show_capa_manager()
